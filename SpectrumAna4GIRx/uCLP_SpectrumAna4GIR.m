...
%     end
    ...
    idx = setdiff([1:record_size],time_idx{1});
...
%         nseconds1    = Tmpnseconds{1}(1:lch{1});
        years1       = Tmpyears{1}(idx);
        months1      = Tmpmonths{1}(idx);
        days1        = Tmpdays{1}(idx);
        hours1       = Tmphours{1}(idx);
        minutes1     = Tmpminutes{1}(idx);
        seconds1     = Tmpseconds{1}(idx);
        nseconds1    = Tmpnseconds{1}(idx); 
    elseif Iint > 1 && ~MemorySaver
        years1       = [ years1,     Tmpyears{1}(idx) ];
        months1      = [ months1,    Tmpmonths{1}(idx) ] ;
        days1        = [ days1,      Tmpdays{1}(idx) ];
        hours1       = [ hours1,     Tmphours{1}(idx) ];
        minutes1     = [ minutes1,   Tmpminutes{1} ];
        seconds1     = [ seconds1,   Tmpseconds{1}(idx) ];
        nseconds1    = [ nseconds1,  Tmpnseconds{1}(idx) ];
	end
