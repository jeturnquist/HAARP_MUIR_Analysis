...
%
%          ver.1.0: Aug-9-2008 
%          ver.1.1: Mar-9-2009 : Fixed double tick marks
%
...
%         StartTimeChar = StartTimeStruct.str;
    ...
    %     set(gca, 'XTickLabel', TimeTick);
        ...
        %%% First y-axis: Altitude; range corrected for beam elevation (km) (km)
        set(ax(1), 'Ylim',[AltTick(1), AltTick(end)])
        set(ax(1), 'YTick', AltTick);
        set(ax(1), 'YTickLabel', round((AltTick/100)/10));
        ylabel({['Altitude (km)'];['']});
        colormap(jet);
        colorbar('location', 'eastoutside');    
        caxis([30 50]);
 
        %%% Second y-axis; Range (km)     
        ...
        set(ax(2),'YLim',[AltTick(1), AltTick(end)]);
        set(ax(2), 'YTick', AltTick);
        set(ax(2), 'YTickLabel', round((RangeTick/100)/10));
        ylabel(ax(2), ['Range (km)']);
        
%         set(ax(2), 'YTickLabel', 'off')
%         set(ax(2), 'YTick', [])
