...
if ILCh
    fseek(fid, abs((ILCh-chid))*record_bytes, -1); 
    chid = ILCh;
else
    fseek(fid, 0, -1);
end
...
kk = 0;
 for ii = 1:num_records
     ...
     fseek(fid,40,0);
     tmp_freq = fread(fid,1,'int');
     
     if tmp_freq == CH_FREQ{FitChannel4IL}
         chid = FitChannel4IL;
     elseif tmp_freq == CH_FREQ{FitChannel4UPL}
         chid = FitChannel4UPL;
     elseif tmp_freq == CH_FREQ{FitChannel4DPL}
         chid = FitChannel4DPL;
     end
     fseek(fid,-44,0);
