...
%% Subplot creation function
  function mysubplot(ind)
    ...
    % Create the axes
    axesHdl(ind) = subplot(axesNb,1,ind,'Parent',f1);
    
    % Get axes position and modify y-pos and height
    axPos = get(axesHdl(ind),'Position');
    axPos([2 4]) = [.1 + .003 + axH * (indA-1)  ,  axH-0.003];
    set(axesHdl(ind),'Units','normalized','Position',axPos);
