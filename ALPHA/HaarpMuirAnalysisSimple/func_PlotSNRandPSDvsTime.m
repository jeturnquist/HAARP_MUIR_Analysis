...
%     rng_spacing = floor(length(rng_idx)*10); 
    rng_spacing = floor((rng_idx(end) - rng_idx(1)) / 10);
%     rng_spcaing = 10;
    ...
    FreqTick  = round(FreqArr(1:100:end));
...
%     set(gca, 'XTickLabel', TimeTick);
    ...
    set(gca, 'YTickLabel', round((AltTick/100)/10));
    ylabel({['Altitude (km)'];['']});
    colormap(jet);
...
%     set(gca, 'YTickLabel', FreqTick);
    ...
    TimeChar  = [TmpChar1.vec(1:end),'-',TmpChar2.vec(5:end)];
